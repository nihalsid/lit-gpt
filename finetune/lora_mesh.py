...
def train(fabric, model, optimizer, config, speed_monitor):
    ...
    train_data = NgonSoup(tokenizer, config, 'train', config.scale_augment, config.shift_augment)
    val_data = NgonSoup(tokenizer, config, 'val', config.scale_augment_val, config.shift_augment_val)
